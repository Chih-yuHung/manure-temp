...
obs.VA<-read.csv(paste(result,"temp.no.daily.csv",sep=""),header=T) 

#simulated data before calibration
sim.VA.og<-read.csv(paste(result,"VA/original/",Location,Sys.Date(),".csv",sep=""),header=T) 
SR.og<-sim.VA.og$total.radiation[1096:1460]/12/277.77778
#simulated data after calibration and modification
sim.VA<-read.csv(paste(result,"VA/with shade/",Location,Sys.Date(),".csv",sep=""),header=T)
SR<-sim.VA$total.radiation[1096:1460]/12/277.77778
#Draw the last year only
sim.VA.og<-sim.VA.og[c(1096:1460),]
sim.VA<-sim.VA[c(1096:1460),]

#For measured manure temperature
png(file=paste(result,"VA/figures/",Location,Sys.Date(),".png",sep="")
    ,width=800, height =600)
par(mar=c(3,4,2,4))
plot(temp,type="l",xaxt='n',col="green",ylim=c(-15,30)
     ,xlab="Date",ylab="Temperature",las=1) #Air temperature
lines(obs.VA$temp.avg,type="l") #manure avg. obs temperature
lines(sim.VA.og$Temperature.C,type="l",col="blue",lwd=2) #without shade calibration
lines(sim.VA$Temperature.C,col="red",lwd=2)
...
Axis(side=1, at=c(14,106,198,288),las=1
     ,labels=c("Jul. 1, 2020","Oct. 1, 2020","Jan. 1, 2020","Apr. 1, 2021"))
...
text(20,28,"Vallentuna (VA) site")
dev.off() 

#For manure depth
png(file=paste(result,"VA/figures/",Location,Sys.Date(),"_depth.png",sep="")
    ,width=800, height =600)
plot(sim.VA$Depth.cm,type="l"
     ,ylim=c(0,350),xaxt='n'
     ,col="black",xlab="Date"
     ,ylab="Depth (cm)")
